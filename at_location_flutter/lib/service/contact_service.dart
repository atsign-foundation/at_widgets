...
AtContact? getCachedContactDetail(String? atsign) {
  ...
  if (KeyStreamService().contactList.isNotEmpty) {
    int index = KeyStreamService().contactList.indexWhere((AtContact element) => element.atSign == atsign);
...
Future<Map<String, dynamic>> getContactDetails(String? atSign) async {
  ...
  try {
    ...
    AtValue result = await KeyStreamService().atClientInstance!.get(key).catchError((dynamic e) {
